...
const TutorQueries = {
  ...
  tutorMedianRating: {
    ...
    async resolve(_, args, context) {
      ...
      const tutor = await Tutor.findById(args.id);
      if (!tutor) throw new Error("Tutor not found");
      return calculateMedian(tutor.ratings.map(r => r.rating));
    },
  },
};
...
const TutorMutations = {
  ...
  updateTutor: {
    ...
    async resolve(_, args, context) {
      ...
      if (args.password !== undefined) {
        updateFields.password = await bcrypt.hash(args.password, 10);
      }
  ...
  rateTutor: {
    ...
    async resolve(_, { id: tutorId, rating, studentId }, context) {
      await protect(context);
      ...
      // only past, confirmed, paid appointments count
      const pastConfirmedPaid = await Appointment.find({
        tutor:     tutorId,
        student:   studentId,
        confirmed: true,
        isPaid:    true,         // ← require payment
        date:      { $lt: now }, // ← strictly before now
      });

      if (pastConfirmedPaid.length === 0)
        throw new Error("You may only rate after a confirmed & paid session has occurred.");

      const tutor = await Tutor.findById(tutorId);
      if (!tutor) throw new Error("Tutor not found");

      const given = tutor.ratings.filter(r => r.studentId.toString() === studentId).length;
      if (given >= pastConfirmedPaid.length)
        throw new Error("You have already rated for all past paid sessions.");
